...
module.exports = {
    login: async (req, res, next) => {
        try {
            ...
            const defaultAmount = parseInt(process.env.DEFAULT_POINT_AMOUNT) || 0;
            const defaultProfitPerHour = parseInt(process.env.DEFAULT_PROFIT_PER_HOUR) || 0;

            const point = await prisma.point.findFirst({ where: { player_id: player.id } });
            if (!point) {
            ...
            const token = jwt.sign({ ...player, role: 'player' }, process.env.JWT_SECRET, { expiresIn: '1d' });
    ...
    whoami: async (req, res, next) => {
        try {
            ...
            const point = await prisma.point.findFirst({ where: { player_id: req.user.id } });
            if (point) {
